...
| `src/core/loop.ts` | - | ループ内でのHat切り替えは将来実装予定 | - |
| `src/input/prompt.ts` | - | Hat指示のプロンプト注入は将来実装予定 | - |

### 7.2 統合確認コマンド

```bash
# HatSystem の使用箇所を確認
grep -rn 'HatSystem\|HatDefinition\|EventBus' src/

# 期待される出力:
# src/core/hat.ts: 実装ファイル
# src/core/event.ts: EventBus実装
# src/core/loop.ts: EventBus使用
```
...
| 統合ポイント | ステータス | 備考 |
|-------------|----------|------|
| `LoopEngine` でのEventBus連携 | ✅ 実装済み | イベント発行のみ |
| Hat切り替えロジック | ❌ 未実装 | v3.0.0では未統合 |
| プロンプトへのHat指示注入 | ❌ 未実装 | v3.0.0では未統合 |
