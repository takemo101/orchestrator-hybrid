...
|------------------|-------------|------|----------|
| `src/cli.ts` (`run` コマンド) | `new PromptGenerator()`, `generate()` | Issue情報からプロンプトを生成 | - |

### 6.2 統合確認コマンド

```bash
# PromptGenerator の使用箇所を確認
grep -rn 'PromptGenerator\|generate(' src/

# 期待される出力:
# src/cli.ts: new PromptGenerator(), promptGenerator.generate() 呼び出し
# src/input/prompt.ts: 実装ファイル
```
